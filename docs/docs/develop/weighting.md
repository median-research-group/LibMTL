...
```eval_rst
There are four important functions in :class:`LibMTL.weighting.AbsWeighting`. We will introduce them in detail as follows.

- :func:`backward`: The main function of a weighting strategy whose input and output format can be found in :func:`LibMTL.weighting.AbsWeighting.backward`. To rewrite this function, you need to consider the case of ``multi-input`` and ``multi-label`` (refer to `here <../user_guide/mtl.html#network-architecture>`_) and the case of ``rep-grad`` and ``param-grad`` (refer to `here <../user_guide/mtl.html#weighting-strategy>`_) if you want to combine your weighting method with more architectures or apply your method to more datasets.
...
The :func:`_get_grads` and :func:`_backward_new_grads` functions are very useful to rewrite the :func:`backward` function and you can find more details in `here <../../_modules/LibMTL/weighting/abstract_weighting.html#AbsWeighting>`_.
```
