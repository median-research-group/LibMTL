...
def template(yaml_path='./SI.yaml', w_dict=None, arch_dict=None, raises=False):
...
# arch
def test_CGC():
    template(arch_dict={'name': 'CGC', 'num_experts': [1, 1, 1, 1], 'img_size': [3, 288, 384]})

def test_MMoE():
    template(arch_dict={'name': 'MMoE', 'num_experts': [2], 'img_size': [3, 288, 384]})

def test_Cross_stitch():
    template(arch_dict={'name': 'Cross_stitch'})

def test_DSelect_k():
    template(arch_dict={'name': 'DSelect_k', 'num_experts': [2], 
                                              'img_size': [3, 288, 384], 'num_nonzeros': 1, 'kgamma': 1.0})

def test_MTAN():
    template(arch_dict={'name': 'MTAN'})

def test_PLE():
    template(arch_dict={'name': 'PLE', 'num_experts': [1, 1, 1, 1], 'img_size': [3, 288, 384]})
